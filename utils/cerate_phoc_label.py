...
def build_phoc(words, phoc_unigrams, unigram_levels,
    ...
    phocs = np.zeros((len(words), phoc_size), dtype=int)
    ...
    for word_index, word in enumerate(words):
        ...
        # add bigrams
        if phoc_bigrams is not None:
            ngram_features = np.zeros(len(phoc_bigrams) * np.sum(bigram_levels), dtype=int)
...
def unigrams_from_word_list(word_label):
    unigrams = [elem for word in word_label for elem in word if elem.isdigit() or elem.isalpha()]
    unigrams = list(sorted(set(unigrams)))
    return unigrams


def unigrams_from_new_historical_mongolian_word_list(word_label):
    unigrams = [elem for word in word_label for elem in word]
    unigrams = list(sorted(set(unigrams)))
    return unigrams


def unigrams_from_all_label(word_label):
    unigrams = [elem for word in word_label for elem in word]
    unigrams = list(sorted(set(unigrams)))
    return unigrams
